...
export default function ServiceBooking({ professional, user, onBack, onBookingComplete }: ServiceBookingProps) {
  ...
  const [selectedDuration, setSelectedDuration] = useState<number>(professional.fixedDuration || professional.minDuration || 60)
  ...
  const createMercadoPagoPreference = async () => {
    ...
    const preferenceData = {
      items: [
        {
          title: `${professional.service} - ${professional.name}`,
          unit_price: currentPrice,
          quantity: 1,
          description: `Servicio programado para ${selectedDate?.toLocaleDateString('es-ES')} a las ${selectedTimeSlot?.start}`
        }
      ],
      payer: {
        email: "cliente@email.com" // En producción vendría del usuario logueado
      },
      back_urls: {
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6 mb-8">
          <div className="flex items-center justify-between">
            <div className={`flex items-center gap-3 ${bookingStep === 'datetime' ? 'text-blue-600' : bookingStep !== 'datetime' ? 'text-green-600' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-medium ${
                bookingStep === 'datetime' ? 'border-blue-600 bg-blue-50' : 
                bookingStep !== 'datetime' ? 'border-green-600 bg-green-50' : 'border-gray-300'
              }`}>
            ...
            <div className={`w-12 h-0.5 ${bookingStep === 'details' || bookingStep === 'payment' ? 'bg-green-600' : 'bg-gray-200'}`}></div>
            
            <div className={`flex items-center gap-3 ${bookingStep === 'details' ? 'text-blue-600' : bookingStep === 'payment' ? 'text-green-600' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-medium ${
                bookingStep === 'details' ? 'border-blue-600 bg-blue-50' : 
                bookingStep === 'payment' ? 'border-green-600 bg-green-50' : 'border-gray-300'
              }`}>
                {bookingStep === 'payment' ? <CheckCircle className="w-5 h-5 text-green-600" /> : '2'}
              </div>
              <span className="font-medium">Detalles</span>
            </div>
            
            <div className={`w-12 h-0.5 ${bookingStep === 'payment' ? 'bg-green-600' : 'bg-gray-200'}`}></div>
            
            <div className={`flex items-center gap-3 ${bookingStep === 'payment' ? 'text-blue-600' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-medium ${
                bookingStep === 'payment' ? 'border-blue-600 bg-blue-50' : 'border-gray-300'
              }`}>
                3
              </div>
